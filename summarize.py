  ...
  """
    ...
    for chunk in tqdm(text_chunks):
        if summarize_recursively and accumulated_summaries:
            ...
            chain = summarize_template(tokenizer, additional_instructions, accumulated=True) | llm.bind(
                stop=["<|eot_id|>"])
            response = chain.invoke({'accumulated_summaries_string': accumulated_summaries_string, 'chunk': chunk})
        else:
            # Directly passing the chunk for summarization without recursive context
            chain = summarize_template(tokenizer, additional_instructions, accumulated=False) | llm.bind(
                stop=["<|eot_id|>"])
            response = chain.invoke({'chunk': chunk})
        if response.startswith('assistant\n\n'):
            response = response.replace('assistant\n\n', '')
        if response.startswith('Here is a summarized version of the text:\n'):
            response = response.replace('Here is a summarized version of the text:\n', '')
        if response.startswith('Here is a summarized version of the text, focusing only on the first example:\n'):
            response = response.replace('Here is a summarized version of the text, focusing only on the first example:\n', '')
